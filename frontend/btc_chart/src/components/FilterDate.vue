  ...
  export default {
    ...
    methods: {
      setDefaultDateRange() {
        ...
        pastDate.setDate(today.getDate() - 7); // Default to last 7 days
      ...
      async fetchCandlestickData() {
        ...
        try {
          ...
          this.chartData = response.data.map((item) => [
            item.timestamp,
            item.close, // Use close price for wave representation
          ]);
  
          if (this.chartData.length === 0) {
            console.error("No data available. Chart won't render.");
            return;
          }
          ...
          series: [
            {
              name: "Wave Chart",
              type: "line",
              smooth: true, // Smooth curves to create waves
              data: this.chartData,
              lineStyle: {
                color: "#00aaff",
                width: 2,
              ...
              areaStyle: {
                color: "rgba(0, 170, 255, 0.3)", // Light blue gradient fill
              },
            },
          ],
        };
